...
IMAPClient.prototype._processCommandQueue = function(){
    ...
    if(this.debug){
        this.log("CLIENT: "+ (command.data || "").trim());
    }
        ...
        if(this._xoauth2 && this._xoauth2RetryCount && this._xoauth2RetryCount<3){
            this._xoauth2.generateToken((function(err, token){
                if(err){
                    ...
                    err.errorLog = this._log.slice(0, this._logLength);
                    this.emit("error", err);
                    return;
                }
                this._postCapability();
...
IMAPClient.prototype._formatEnvelope = function(envelopeData){
    ...
    if(dataObject["X-GM-THRID"]){
     ...
     if(dataObject["X-GM-MSGID"]){
        message.xGMMsgId = dataObject["X-GM-MSGID"];
    }
    ...
    if(dataObject.BODY && (dataObject.BODY = (dataObject.BODY || "").toString().trim())){
        headers = mimelib.parseHeaders(dataObject.BODY);
        if(headers.references){
            message.references = [].concat.apply([], headers.references.map(function(row){
                return row.split(/>\s*</).map(function(reference){
                    return "<"+reference.replace(/[\s<>]/g,"")+">";
                });
            }));
        }

        if(headers['thread-index'] && Array.isArray(headers['thread-index'])){
            message.threadIndex = [].concat.apply([], headers['thread-index'].map(function(row){

                var thread = new Buffer(row, "base64"),
                    threads = [thread.slice(0, 22).toString("hex")],
                    pos = 22, i = 0;

                while(pos < thread.length){
                    threads.push(threads[i++] + thread.slice(pos, pos + 5 < thread.length ? pos + 5 : thread.length).toString("hex"));
                    pos += 5;
                }

                threads = threads.map(function(id){
...
IMAPClient.prototype._checkNewMail = function(){
    ...
    this._send("UID FETCH "+this._selectedMailbox.UIDNext+":* (FLAGS ENVELOPE BODY[HEADER.FIELDS (REFERENCES THREAD-INDEX)]" +(this._capabilities.indexOf("X-GM-EXT-1")>=0?" X-GM-LABELS X-GM-THRID":"")+ ")", (function(){
        this._checkForNewMail = false;
    }).bind(this),
    (function(){
...
IMAPClient.prototype.search = function(query, callback){
    this._uids = [];
    if(this._currentState != this.states.SELECTED){
        if(typeof callback == "function"){
            callback(new Error("No mailbox selected"));
 ...
 * @param {Number} uid Message identifier
 */
IMAPClient.prototype.createMessageStream = function(uid, callback){
    ...
    this._send("UID FETCH "+uid+":"+uid+" (ENVELOPE BODY.PEEK[] X-GM-LABELS X-GM-THRID X-GM-MSGID)", (function(status){
        this._collectMailList = false;
        this._literalStreaming = false;
...
IMAPClient.prototype.idle = function(){
    if(this._capabilities.indexOf("IDLE")>=0){
        ...
        if(this.debug){
            this.log("WARNING: Server does not support IDLE, fallback to NOOP");
        }
